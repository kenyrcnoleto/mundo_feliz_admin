class PedidoProdutosController < ApplicationController
  ...
  def index
    #preciso buscar somentes os itens que estão relacionados ao pedido_id
  ...
  def create
    ...
    #precisa configurar tb na hora de salvar
    @pedido_produto.pedido = @pedido

    respond_to do |format|
      if @pedido_produto.save
        #Necessário colocar a rota auterada também no redirec do controller
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to pedido_pedido_produtos_path(@pedido), status: :see_other, notice: "Pedido produto was successfully destroyed." }
  ...
  private
    ...
    #Filtro para todas as ações a instancia de pedido tambem deve ser válida
    def set_pedido
